...
/**
 * These constants are returned by sqlite3_vtab_on_conflict() to inform a virtual table
 * implementation of the ON CONFLICT mode for the SQL statement being evaluated.
 *
 * [Conflict resolution modes](https://sqlite.org/c3ref/c_fail.html)
 * [The ON CONFLICT Clause](https://sqlite.org/lang_conflict.html)
 */
public enum class Sqlite3ConflictResolutionMode(internal val code: Int) {
     ...
     * row that contains the constraint violation and continues processing subsequent rows of the
     * SQL statement as if nothing went wrong. Other rows before and after the row that contained
     * the constraint violation are inserted or updated normally. No error is returned for
     * uniqueness, NOT NULL, and UNIQUE constraint errors when the IGNORE conflict resolution
     * algorithm is used. However, the IGNORE conflict resolution algorithm works like ABORT for
     * foreign key constraint errors.
     */
    IGNORE(Sqlite3AuthorizerCode.IGNORE.code),
     ...
     */
    ABORT(Sqlite3Result.ABORT.code),

    /**
     * When a UNIQUE or PRIMARY KEY constraint violation occurs, the REPLACE algorithm deletes
     * pre-existing rows that are causing the constraint violation prior to inserting or updating
     * the current row and the command continues executing normally. If a NOT NULL constraint
     * violation occurs, the REPLACE conflict resolution replaces the NULL value with the default
     * value for that column, or if the column has no default value, then the ABORT algorithm is
     * used. If a CHECK constraint or foreign key constraint violation occurs, the REPLACE conflict
